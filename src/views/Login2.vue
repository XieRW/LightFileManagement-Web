    ...
    export default defineComponent({
        ...
        setup() {
            ...
            function routeSkip(item) {
                const paths = sessionStorage.getItem('route') ?? '';
                ...
                ($http )(requestData.value)
                    .then((res) => {
                        if (res.code === 0) {
                            ...
                            localStorage.setItem('legacyH5URL', (window ).config.legacyH5URL);
                            const data = {
                                id: route.query.id,
            ...
            function getPathParams() {
                ...
                const paramLabel = ref(''); // 字段名称
                const paramValue = ref(0); // 字段值
                const paramType = ref(0); // paramType: 1是任务  2是响应  3是事件
                ...
                // 保存登录链接的参数（对接2.7H5）
                const affairsId = route.query.affairsId ? route.query.affairsId : '';
                localStorage.setItem('affairsId', affairsId);
                const contactorId = route.query.contactorId ? route.query.contactorId : '';
                localStorage.setItem('contactorMap', contactorId);
                const approvalId = route.query.approvalId ? route.query.approvalId : '';
                localStorage.setItem('approvalMap', approvalId);
                // // 11.28新增 事件快报
                const eventDealBulletinId = route.query.eventDealBulletinId ? route.query.eventDealBulletinId : '';
                // store.commit('saveBulletin', eventDealBulletinId);
                localStorage.setItem('eventDealBulletinId', eventDealBulletinId);
                // // 值班点名
                const contactRelId = route.query.contactRelId ? route.query.contactRelId : '';
                const groupCode = route.query.groupCode ? route.query.groupCode : '';
